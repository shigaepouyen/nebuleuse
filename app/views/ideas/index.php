        ...
        <?php else: ?>
            <?php foreach ($ideas as $idea): ?>
                <div class="bg-white p-4 rounded-lg shadow flex justify-between items-start">
                    <div>
                        <h3 class="font-bold text-gray-800"><?= htmlspecialchars($idea['title']) ?></h3>
                        <?php if (!empty($idea['notes'])): ?>
                            <p class="text-gray-600 text-sm mt-1"><?= nl2br(htmlspecialchars($idea['notes'])) ?></p>
                        <?php endif; ?>
                        <small class="text-gray-400 mt-2 block">Créée le <?= date('d/m/Y', strtotime($idea['created_at'])) ?></small>
                    </div>
                    <div class="flex items-center gap-2 flex-shrink-0 ml-4">
                        <a href="/ideas/<?= $idea['id'] ?>/convert" class="text-gray-400 hover:text-indigo-600" title="Convertir en tâche">
